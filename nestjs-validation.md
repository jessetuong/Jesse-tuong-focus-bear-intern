...
ValidationPipe validates incoming data against DTO rules before it reaches the application’s business logic. This prevents malformed or unexpected data from being processed. Options such as whitelist and forbidNonWhitelisted can also prevent clients from sending properties that aren’t part of the API’s expected data structure.

## What is the difference between built-in and custom pipes?

Built-in pipes are provided by NestJS for common validation and transformation tasks, such as ValidationPipe and ParseIntPipe. Custom pipes are created by developers when application-specific validation or transformation logic is required.
