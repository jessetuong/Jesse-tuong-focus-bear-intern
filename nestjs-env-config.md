1. ow does @nestjs/config help manage environment variables?
...
- Secrets stored in source code can accidentally be committed to version control and exposed to other developers or attackers. Once a secret has been committed to a repository, removing the file does not necessarily remove it from the repository’s history. Environment variables and dedicated secret-management systems provide a safer way to supply sensitive configuration to an application.
